...
// КЛЮЧ GEMINI API (Если ошибка 'Quota' не исчезнет, создайте НОВЫЙ ключ на другой Gmail аккаунт)
...
const App = () => {
  ...
  // --- ЛОГИКА ГЕНЕРАЦИИ (ПОДДЕРЖКА БОЛЬШИХ ТЕКСТОВ И ИСПРАВЛЕНИЕ ОШИБОК) ---
  const handleGenerateTest = async (existing = null) => {
    ...
    setIsLoading(true);
    // Поддержка до 150 000 символов (~40 страниц)
    const safeText = text.substring(0, 150000); 

    try {
      // Используем стабильный endpoint без двойных префиксов
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          contents: [{ 
            parts: [{ text: `ТЫ: Профессор медицины. ЗАДАНИЕ: Составь 30 тестовых вопросов по тексту ниже. Ответ дай СТРОГО в формате JSON (массив объектов), без вводных слов. Каждый вопрос имеет 4 варианта и 1 верный индекс. ТЕКСТ: ${safeText}` }] 
          }],
          generationConfig: { 
            temperature: 0.1, 
      ...
      if (!res.ok) {
        ...
        if (res.status === 429 || (data.error && data.error.message.includes("quota"))) {
          throw new Error("Google заблокировал этот ключ за превышение лимитов. Подождите 24 часа или используйте НОВЫЙ ключ с другого Gmail-аккаунта.");
        }
        if (data.error && data.error.message.includes("not found")) {
          throw new Error("Ошибка конфигурации модели. Пожалуйста, проверьте настройки API.");
        }
        throw new Error(data.error?.message || "Ошибка сервера ИИ");
      }

      const rawContent = data.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!rawContent) throw new Error("ИИ вернул пустой ответ. Текст слишком большой или сложный.");
      ...
      showToast("Тест из 30 вопросов создан!");
  ...
  const renderCurrentView = () => {
    if (!isFirebaseReady) return <div className="min-h-screen w-full flex items-center justify-center bg-slate-950 text-white p-10 text-center">Firebase Config Error.</div>;
    if (!user) return <div className="min-h-screen w-full flex items-center justify-center bg-slate-950 text-white font-black animate-pulse">ВХОД...</div>;

    switch (view) {
      case 'welcome': return (
        <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white rounded-[2.5rem] p-10 shadow-2xl text-center flex flex-col items-center">
            ...
            <p className="text-slate-400 font-bold text-[9px] uppercase tracking-widest mb-10 opacity-60">Аттестационный портал</p>
            <div className="space-y-4 w-full">
              <input type="text" value={studentName} onChange={(e) => setStudentName(e.target.value)} placeholder="ФИО студента" className="w-full p-5 bg-slate-50 border-2 rounded-2xl outline-none focus:border-emerald-500 font-bold text-slate-800 text-center" />
              ...
              <button onClick={() => setView('admin-login')} className="text-slate-400 hover:text-emerald-600 text-[9px] font-black uppercase mt-4 block w-full tracking-widest">Админка</button>
            </div>
          </div>
        </div>
      );
      ...
      case 'menu': return (
        <div className="min-h-screen w-full bg-slate-950 flex flex-col items-center justify-center p-4 gap-12 text-center">
          <h2 className="text-white text-3xl font-black uppercase tracking-tighter">Выберите раздел</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 w-full max-w-4xl text-left">
            <button onClick={() => setView('student-select-test')} className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-4 border-transparent hover:border-emerald-500 transition-all group flex flex-col items-center sm:items-start">
              <ClipboardList className="text-emerald-600 w-12 h-12 mb-8" />
            ...
            <button onClick={() => setView('student-select-tasks')} className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-4 border-transparent hover:border-blue-500 transition-all group flex flex-col items-center sm:items-start">
              <Stethoscope className="text-blue-600 w-12 h-12 mb-8" />
      ...
      case 'admin-login': return (
        <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-4 text-center">
          <div className="max-w-md w-full bg-white rounded-[3rem] p-12 shadow-2xl flex flex-col items-center">
            ...
            <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="••••" className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-black text-center text-slate-900 tracking-[1em] text-3xl mb-10 shadow-inner" />
            <button onClick={() => adminPassword === ADMIN_PASSWORD_SECRET ? (setIsAdminAuthenticated(true), setView('admin')) : showToast("Неверный код")} className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black uppercase shadow-xl active:scale-95 transition-all text-xs tracking-widest">Войти</button>
          ...
          <div className="max-w-7xl mx-auto flex flex-col">
            <div className="flex flex-col md:flex-row justify-between items-center gap-10 mb-16 text-center md:text-left">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                ...
                <button onClick={() => {setIsAdminAuthenticated(false); setView('welcome');}} className="bg-white text-slate-400 px-6 py-5 rounded-xl text-[10px] font-black border-2 border-slate-100 hover:text-red-500">Выход</button>
              </div>
            </div>
            <div className="bg-white rounded-[4rem] shadow-xl overflow-hidden border border-slate-100 flex flex-col">
              <div className="p-10 bg-slate-50/50 border-b border-slate-100 flex justify-between items-center text-center">
                            ...
                            <div className="flex items-center gap-8 flex-1">
                                ...
                                <div className="text-left">
                                  ...
                                  <div className="flex gap-6 items-center">
                                    ...
                                    <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-lg ${m.isVisible ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>{m.isVisible ? "ОТКРЫТ" : "СКРЫТ"}</span>
                                  </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4 text-right">
                                <button disabled={isLoading} onClick={() => handleGenerateTest(m)} className="px-8 py-5 bg-slate-100 text-slate-600 rounded-[1.5rem] font-black text-[10px] uppercase flex items-center gap-3 active:scale-95 hover:bg-emerald-50 border border-slate-200">{isLoading ? <Loader2 className="animate-spin w-4 h-4 text-center"/> : <RefreshCw className="w-4 h-4 text-center"/>} ОБНОВИТЬ ИИ</button>
                                <button onClick={() => { if(confirm("Удалить этот тест?")) deleteDoc(doc(db, 'artifacts', PORTAL_ID, 'public', 'data', 'materials', m.id)); }} className="p-6 bg-red-50 text-red-500 rounded-[1.5rem] hover:bg-red-500 hover:text-white transition-all"><Trash2 className="w-6 h-6 text-center" /></button>
                            </div>
                        </div>
                    ))}
                </div>
        ...
        <div className="min-h-screen w-full bg-slate-50 p-6 md:p-12 text-left">
            <div className="max-w-6xl w-full mx-auto flex flex-col">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mb-16 gap-6">
                  <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tighter">Библиотека задач</h2>
                        ...
                        <div key={s.id} className="bg-white border border-slate-100 rounded-[3rem] p-10 shadow-xl flex flex-col md:flex-row justify-between items-center gap-10 group hover:border-blue-300 transition-all text-left">
                            <div className="flex items-center gap-8 flex-1 text-left">
                                <div onClick={() => updateDoc(doc(db, 'artifacts', PORTAL_ID, 'public', 'data', 'task_sections', s.id), { isVisible: !s.isVisible })} className={`cursor-pointer w-20 h-20 rounded-[2rem] flex items-center justify-center transition-all ${s.isVisible ? 'bg-blue-600 text-white shadow-xl shadow-blue-500/20' : 'bg-slate-100 text-slate-400 border-2 border-slate-200'} hover:scale-105 active:scale-95`}>
                                    {s.isVisible ? <Unlock className="w-8 h-8 text-center" /> : <Lock className="w-8 h-8 text-center" />}
                                </div>
                                <div className="text-left">
                            ...
                            <div className="flex items-center gap-4 text-right">
                                <button onClick={() => updateDoc(doc(db, 'artifacts', PORTAL_ID, 'public', 'data', 'task_sections', s.id), { isAnswersEnabled: !s.isAnswersEnabled })} className={`p-6 rounded-2xl transition-all ${s.isAnswersEnabled ? 'bg-emerald-500 text-white shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}>
                                  {s.isAnswersEnabled ? <Eye className="w-6 h-6 text-center" /> : <EyeOff className="w-6 h-6 text-center" />}
                                </button>
                                <button onClick={() => { if(confirm("Удалить блок задач?")) deleteDoc(doc(db, 'artifacts', PORTAL_ID, 'public', 'data', 'task_sections', s.id)); }} className="p-6 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all"><Trash2 className="w-6 h-6 text-center" /></button>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      case 'setup-test': return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
            <div className="max-w-5xl w-full bg-white rounded-[4rem] p-12 sm:p-20 shadow-2xl relative text-center animate-in slide-in-from-bottom-10 flex flex-col items-center">
                <button onClick={() => setView('admin')} className="absolute top-12 left-12 text-slate-400 font-black uppercase text-[10px] flex items-center gap-3 hover:text-slate-900 transition-all self-start"><ArrowLeft className="w-5 h-5" /> Назад</button>
                <div className="bg-emerald-100 w-24 h-24 rounded-3xl mb-10 flex items-center justify-center"><Plus className="w-12 h-12 text-emerald-600 text-center"/></div>
                <h2 className="text-4xl font-black text-slate-900 uppercase mb-2 tracking-tight">Создание ИИ Теста</h2>
                <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mb-12">Вставьте текст лекции (до 30 страниц), и нейросеть создаст 30 вопросов</p>
                <div className="space-y-6 text-left w-full">
                    ...
                    <textarea value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Вставьте учебный материал..." className="w-full h-[400px] p-10 bg-slate-50 border-2 border-slate-100 rounded-[3rem] focus:bg-white focus:border-emerald-600 outline-none resize-none font-bold text-slate-700 text-lg shadow-inner scrollbar-hide text-left" />
                </div>
      ...
      case 'setup-tasks': return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
            <div className="max-w-5xl w-full bg-white rounded-[4rem] p-12 sm:p-20 shadow-2xl relative text-center animate-in slide-in-from-bottom-10 flex flex-col items-center">
                <button onClick={() => setView('admin-tasks-list')} className="absolute top-12 left-12 text-slate-400 font-black uppercase text-[10px] flex items-center gap-3 hover:text-slate-900 transition-all self-start"><ArrowLeft className="w-5 h-5" /> Назад</button>
      ...
      case 'student-select-test': return (
        <div className="min-h-screen w-full bg-slate-950 p-6 flex flex-col items-center text-left">
          <div className="max-w-4xl w-full text-left text-left">
            <button onClick={() => setView('menu')} className="text-slate-400 font-black text-[10px] uppercase mb-10 flex items-center gap-2 hover:text-white transition-colors text-left text-left"><ChevronLeft className="w-4 h-4 text-left" /> Назад</button>
            ...
            <div className="grid gap-4 text-left">
              {materials.filter(m => m.isVisible).map(m => (
                <button key={m.id} onClick={() => { setActiveMaterial(m); setStudentAnswers([]); setCurrentQuestionIndex(0); setView('quiz'); }} className="bg-slate-800/50 hover:bg-slate-800 p-8 rounded-[2rem] border-2 border-slate-700 flex items-center justify-between group transition-all text-left shadow-lg shadow-black/20">
                  <div className="flex items-center gap-6 text-left">
                    <div className="bg-emerald-500 p-4 rounded-2xl shadow-lg text-center"><ClipboardList className="text-white w-6 h-6 text-center" /></div>
                    <div className="text-left">
                      <h4 className="text-white font-black text-xl uppercase tracking-tight leading-none text-left">{m.title}</h4>
                      <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-2 block text-left">{m.questions?.length} вопросов</span>
                    </div>
                  </div>
                  <ChevronRight className="text-slate-600 group-hover:text-emerald-400 text-left" />
                </button>
              ))}
            </div>
          </div>
      ...
      case 'student-select-tasks': return (
        <div className="min-h-screen w-full bg-slate-950 p-6 flex flex-col items-center text-left text-left">
            <div className="max-w-4xl w-full text-left text-left">
                <button onClick={() => setView('menu')} className="text-slate-400 font-black text-[10px] uppercase mb-10 flex items-center gap-2 hover:text-white transition-colors text-left text-left"><ArrowLeft className="w-4 h-4 text-left" /> Назад</button>
                ...
                <div className="grid gap-4 text-left">
                    {taskSections.filter(t => t.isVisible).map(t => (
                        <button key={t.id} onClick={() => { setActiveTaskSection(t); setCurrentTaskIndex(0); setShowAnswerLocally(false); setView('task-viewer'); }} className="bg-slate-800/50 hover:bg-slate-800 p-8 rounded-[2rem] border-2 border-slate-700 flex items-center justify-between group transition-all text-left shadow-lg shadow-black/20">
                            <div className="flex items-center gap-6 text-left">
                              <div className="bg-blue-500 p-4 rounded-2xl shadow-lg text-center"><Stethoscope className="text-white w-6 h-6 text-center" /></div>
                              <div className="text-left"><h4 className="text-white font-black text-xl uppercase tracking-tight text-left">{t.title}</h4><span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-2 block text-left">{t.tasks?.length} задач</span></div>
                            </div>
                            <ChevronRight className="text-slate-600 group-hover:text-blue-400 text-left" />
                        </button>
                    ))}
                </div>
            </div>
      ...
      case 'quiz':
        ...
        return (
          <div className="min-h-screen w-full bg-slate-950 flex flex-col items-center">
            ...
            <div className="max-w-4xl w-full p-6 flex-1 flex flex-col justify-center text-left text-left">
              <div className="bg-white rounded-[3.5rem] p-12 md:p-16 shadow-2xl relative mb-10 overflow-hidden text-left text-left">
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-12 leading-tight relative z-10 text-left text-left">{q_quiz?.text}</h2>
              ...
              <div className="flex justify-between items-center px-4 w-full max-w-4xl">
                ...
                {currentQuestionIndex === (activeMaterial.questions.length - 1) 
                  ...
                  : <button onClick={() => setCurrentQuestionIndex(p => p + 1)} disabled={!isAns_quiz} className="bg-blue-600 text-white px-12 py-5 rounded-2xl font-black shadow-xl active:scale-95 transition-all uppercase tracking-widest text-sm flex items-center gap-3 text-center">Далее <ArrowRight className="w-5 h-5 text-center" /></button>
                }
              </div>
            </div>
          </div>
        ...
        return (
            <div className="min-h-screen w-full bg-slate-950 flex flex-col items-center text-left">
                <div className="w-full bg-slate-800/80 p-5 border-b border-slate-700 flex justify-between px-10 text-white uppercase font-black text-[10px] sticky top-0 z-50 backdrop-blur-lg text-center">
                    <button onClick={() => setView('student-select-tasks')} className="bg-slate-700 p-2 rounded-xl text-center"><ArrowLeft className="w-5 h-5 text-white text-center" /></button>
                    ...
                    <span className="text-blue-400 text-lg flex items-center text-center">{currentTaskIndex + 1} / {activeTaskSection.tasks.length}</span>
                </div>
                <div className="max-w-5xl w-full p-6 flex-1 flex flex-col justify-center text-left text-left">
                            ...
                            <div className="mt-12 pt-12 border-t border-slate-50 text-left">
                                {showAnswerLocally 
                                    ? <div className="bg-emerald-50 border-2 border-emerald-100 p-10 rounded-[2.5rem] animate-in slide-in-from-top-4 shadow-inner text-left">
                                        <p className="text-emerald-600 font-black uppercase text-[10px] mb-4 tracking-widest flex items-center gap-2 text-left"><CheckCircle2 className="w-4 h-4 text-left"/> Эталон ответа:</p>
                    ...
                    <div className="flex justify-between items-center px-4 mt-10 w-full max-w-5xl">
                        <button disabled={currentTaskIndex === 0} onClick={() => { setCurrentTaskIndex(p => p - 1); setShowAnswerLocally(false); }} className="bg-slate-800 p-6 rounded-3xl text-white font-black uppercase text-xs flex items-center gap-3 hover:bg-slate-700 transition-all shadow-xl text-left"><ArrowLeft className="w-5 h-5 text-left text-left" /> Назад</button>
                        <button disabled={currentTaskIndex === activeTaskSection.tasks.length - 1} onClick={() => { setCurrentTaskIndex(p => p + 1); setShowAnswerLocally(false); }} className="bg-blue-600 p-6 rounded-3xl text-white font-black uppercase text-xs flex items-center gap-3 shadow-2xl shadow-blue-500/20 active:scale-95 transition-all text-left">Вперед <ArrowRight className="w-5 h-5 text-left" /></button>
                    </div>
                </div>
            </div>
        );
      ...
      case 'result': return (
        <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-4 text-center">
          <div className="max-w-2xl w-full bg-white rounded-[5rem] p-20 shadow-2xl relative text-center animate-in zoom-in duration-500 flex flex-col items-center">
            ...
            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mb-12 text-center">Ваш результат сохранен в базе данных</p>
            <div className="grid grid-cols-2 gap-8 mb-16 text-center w-full">
              <div className="bg-emerald-50 p-10 rounded-[3rem] border border-emerald-100 shadow-sm text-center text-center">
                ...
                <p className="text-6xl font-black text-emerald-600 text-center text-center">{(results[0]?.score || 0)} <span className="text-2xl text-emerald-300 font-normal text-center text-center">/ {(results[0]?.total || 0)}</span></p>
              </div>
              <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100 shadow-sm text-center text-center">
                ...
                <p className="text-6xl font-black text-slate-900 text-center text-center">{(results[0]?.percentage || 0)}<span className="text-2xl font-normal opacity-30 text-center">%</span></p>
              </div>
            </div>
            <button onClick={() => setView('menu')} className="w-full bg-slate-900 text-white font-black py-8 rounded-[2.5rem] shadow-2xl hover:bg-slate-800 transition-all uppercase active:scale-95 text-lg text-center text-center">Вернуться в меню</button>
          </div>
        </div>
      );
