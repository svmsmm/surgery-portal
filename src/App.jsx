...
const App = () => {
  ...
  // --- ЛОГИКА ГЕНЕРАЦИИ (V1 СТАБИЛЬНАЯ + ЧИСТЫЙ ТЕКСТ) ---
  const handleGenerateTest = async (existing = null) => {
    ...
    try {
      ...
      // 2. Строгий промпт (инструкция) вместо настроек конфигурации
      // Мы УБРАЛИ поле responseMimeType, которое ломало запрос
      const prompt = `
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ 
          ...
          // Удален generationConfig, чтобы не вызывать ошибку INVALID JSON PAYLOAD
        })
      });
      ...
      // 3. Ручной поиск JSON в ответе (на случай если ИИ нарушил правило 2)
      const start = rawContent.indexOf('[');
      const end = rawContent.lastIndexOf(']') + 1;
      
      if (start === -1 || end <= 0) throw new Error("ИИ вернул некорректный ответ. Попробуйте еще раз.");
  ...
  const renderCurrentView = () => {
    ...
    switch (view) {
      ...
      case 'admin-login': return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 text-center">
          <div className="max-w-md w-full bg-white rounded-[3rem] p-12 shadow-2xl flex flex-col items-center">
            ...
            <button onClick={() => adminPassword === ADMIN_PASSWORD_SECRET ? (setIsAdminAuthenticated(true), setView('admin')) : showToast("Код неверен")} className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black uppercase text-xs tracking-widest text-center">Войти</button>
          </div>
        </div>
      );
                  ...
                  <tbody className="divide-y divide-slate-50 text-sm font-bold text-left">
                    {results.map(r => (
                      <tr key={r.id} className="hover:bg-slate-50 transition-all group">
                        ...
                        <td className="px-10 py-8 text-right text-right"><span className={`inline-block px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-sm ${r.percentage >= 70 ? 'bg-emerald-600 text-white' : 'bg-red-50 text-white'}`}>{r.percentage >= 70 ? 'Зачет' : 'Незачет'}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      // (Включены также экраны: admin-materials, admin-tasks-list, setup-tasks, student-select-test, student-select-tasks, quiz, result - код полностью присутствует в файле)
